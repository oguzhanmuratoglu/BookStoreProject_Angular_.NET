...
export class ShoppingCartService {
  ...
  changeBookQuantityInShoppingCart(bookId: number, quantity: number){
    if(localStorage.getItem("response")){
      this.http.get(`https://localhost:7250/api/ShoppingCarts/ChangeBookQuantityInShoppingCart/${bookId}/${quantity}`)
      .subscribe({
        next: (res:any)=> {
          this.getAllShoppingCarts();
        },
      ...
      if(quantity <= 0){
        ...
        this.http.get(`https://localhost:7250/api/ShoppingCarts/CheckBookQuantityIsAvailable/${bookId}/${quantity}`).subscribe({
          next: (res:any)=> {
  ...
  getAllShoppingCarts(){
    ...
    if(localStorage.getItem("response")){
      this.http.get<SetShoppingCartsModel[]>("https://localhost:7250/api/ShoppingCarts/GetAll/" + this.auth.userId,).subscribe({
        next: (res: any)=> {
  ...
  removeByIndex(index: number) {
    ...
    forkJoin({
      ...
      this.swal.callSwal(res.doYouWantToDeleted, res.cancelBtn, res.confirmBtn, () => {
        if(localStorage.getItem("response")){
          this.http.get("https://localhost:7250/api/ShoppingCarts/RemoveById/" + this.shoppingCarts[index]?.shoppingCartId).subscribe(res=> {
  ...
  payment(data:PaymentModel, callBack: (res: any)=> void){
    this.spinner.show();
    this.http.post("https://localhost:7250/api/ShoppingCarts/Payment", data)
    .subscribe({
      next: (res:any)=> { 
        callBack(res);
        this.spinner.hide();
